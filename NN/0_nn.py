...
y_sol = jCR.func_gSNR_YUK04(jnp.array([8178.0])) / jnp.exp(0.4)
y_grtruth = jnp.log((jCR.func_gSNR_YUK04(x_samples_raw * 1.0e3)) / (y_sol))
y_samples = y_grtruth
...
def loss_forward(y_guess, y_ref):
    ...
    delta = (gamma_guess[0, 0, mask] - gamma_ref[0, 0, mask]) / (jnp.mean(gamma_ref[0, 0, mask]))
...
for epoch in range(N_EPOCHS):
    ...
    if (epoch + 1) % epoch_print == 0:
        ...
        weights, biases = params
        plt.scatter(x_samples_raw, jnp.exp(y_samples) * y_sol, label='Samples')
